...
export class ParameterMappingInput extends React.Component {
    ...
    return (
      ...
      >
        ...
        >
          现有仪表板参数{' '}
          {noExisting ? (
            <Tooltip title="没有与此数据类型对应的仪表板参数">
  ...
  renderInputBlock() {
    ...
    switch (mapping.type) {
      case MappingType.DashboardAddNew:
        return ['关键字', '输入新的参数关键字', this.renderDashboardAddNew()];
      case MappingType.DashboardMapToExisting:
        return [
          '参数名',
          '从现有参数列表中选择',
      ...
      case MappingType.StaticValue:
        return ['数值', null, this.renderStaticValue()];
      default:
        return [];
    }
  }
  ...
  render() {
    ...
    return (
      <Form layout="horizontal">
        <Form.Item label="类型" {...this.formItemProps}>
  ...
  renderContent() {
    ...
    return (
      <div className="parameter-mapping-editor">
        <header>
          编辑参数 <HelpTrigger type="VALUE_SOURCE_OPTIONS" />
        </header>
...
class TitleEditor extends React.Component {
  ...
  renderEditButton() {
    ...
    if (mapping.type === MappingType.StaticValue) {
      return (
        <Tooltip placement="right" title="静态值的标题未出现在窗口小部件中">
  ...
  static getSourceTypeLabel({ type, mapTo }) {
    switch (type) {
      ...
      case MappingType.DashboardMapToExisting:
        return (
          <Fragment>
            可视化面板 <Tag className="tag">{mapTo}</Tag>
          </Fragment>
        );
      case MappingType.WidgetLevel:
        return '小部件参数';
