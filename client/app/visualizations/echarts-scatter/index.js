...
import {
  defaultScatterChartOptions, parseChartType, getChartType, setxAxis, setyAxis,
  setScatter, setThemeColor
} from './echartsScatterChartOptionUtils';


function EchartsScatterRenderer($timeout, $rootScope, $window) {
  return {
    restrict: 'E',
    scope: {
    ...
    link($scope, $element, $route) {
      ...
      if (_.isEmpty($scope.options) || $scope.options.chartType !== "ScatterChart") {
        // console.log("defaultSet");
        $scope.options = defaultScatterChartOptions();
      }
      ...
      const refreshData = () => {
        ...
        try {
          if (!_.isUndefined($scope.queryResult) && $scope.queryResult.getData()) {
            ...
            // 视觉映射范围-若打开写入指定最大最小值，否则unfefined
            if (_.get($scope.options, "visualMap.show",false)) { 
              _.set($scope.options, "visualMap.max", _.get($scope.options, "visualMap_min", "-"));
              _.set($scope.options, "visualMap.min", _.get($scope.options, "visualMap_max", "-"));
            }else{
              _.set($scope.options, "visualMap.max", undefined);
              _.set($scope.options, "visualMap.min", undefined);
            }
          ...
          {c}${_.get($scope.options, "c_Text", "")}`;
            ...
            _.each(_.get($scope.options, "form.yAxisColumns", []), (yAxisColumn) => {
              ...
              const minData = _.min(_.map(_.get($scope.queryResult, "filteredData", []), (row) => {
                return row[yAxisColumn];            
              }))
            ...
            if (document.getElementById("scatter-main")) {
              document.getElementById("scatter-main").id = $scope.options.id;
      ...
      const selectChartType = () => {
        ...
        if (_.get($rootScope, 'selectDECharts', 'n') === 'ECHARTS-SCATTER') {
...
export default function init(ngModule) {
  ngModule.directive('echartsScatterEditor', EchartsScatterEditor);
  ngModule.directive('echartsScatterRenderer', EchartsScatterRenderer);

  ngModule.config((VisualizationProvider) => {
    const renderTemplate =
      '<echarts-scatter-renderer options="visualization.options" query-result="queryResult"  ></echarts-scatter-renderer>';

    const editorTemplate = '<echarts-scatter-editor options="visualization.options" query-result="queryResult"  ></echarts-scatter-editor>';
