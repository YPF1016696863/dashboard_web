...
import { defaultTestChartOptions, getChartType, setData, setThemeColor } from './echartsTestChartOptionUtils';
import color16to10 from '../colorChange';

function EchartsTestRenderer($rootScope) {
    return {
        restrict: 'E',
        scope: {
            queryResult: '=',
        ...
        link($scope, $element) {
            ...
            if (_.isEmpty($scope.options) || $scope.options.chartType !== "TestChart") {
                $scope.options = defaultTestChartOptions();
            }
...
// console.log(dataGauge)
                        ...
                        if (document.getElementById("test-main")) {
                            document.getElementById("test-main").id = $scope.options.id;
...
export default function init(ngModule) {
    ngModule.directive('echartsTestEditor', EchartsTestEditor);
    ngModule.directive('echartsTestRenderer', EchartsTestRenderer);

    ngModule.config((VisualizationProvider) => {
        const renderTemplate =
            '<echarts-Test-renderer options="visualization.options" query-result="queryResult"></echarts-test-renderer>';

        const editorTemplate = '<echarts-Test-editor options="visualization.options" query-result="queryResult"></echarts-test-editor>';
