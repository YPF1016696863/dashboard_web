    ...
    link($scope, elm) {
      ...
      function setBounds() {
        ...
        if (b) {
          ...
          const allMarkers = _.flatten(
            _.map(_.values(layers), l => l.getLayers())
          );
      ...
      function render() {
        ...
        $scope.visualization.options.mapTileUrl =
          $scope.visualization.options.mapTileUrl ||
          '//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
        ...
        if (queryData) {
          ...
          const options = _.map(groupNames, group => {
            if (
              $scope.visualization.options.groups &&
              $scope.visualization.options.groups[group]
            ) {
          ...
          $scope.visualization.options.groups = _.zipObject(
            groupNames,
            options
          );
